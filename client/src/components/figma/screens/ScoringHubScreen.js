...
import { ChevronLeft, Activity, Play, Eye, Plus, CheckCircle, Clock } from 'lucide-react';
...
export function ScoringHubScreen({ onBack, onSelectMatch }) {
  ...
  const filtered =
    filter === 'all'
      ? MOCK_MATCHES
      : MOCK_MATCHES.filter((m) => m.status === filter);

  const liveCount = MOCK_MATCHES.filter((m) => m.status === 'in_progress').length;
  ...
  const getScoreSummary = (match) => {
    ...
    const { sport, scores, status } = match;
  ...
  return (
    <div className="figma-page" style={{ paddingBottom: '5rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        ...
        <span className="figma-heading2" style={{ margin: 0, flex: 1 }}>Scoring</span>
        {liveCount > 0 && (
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
          ...
          <div>
            <div style={{ color: '#fff', fontSize: 16, fontWeight: 700 }}>Score a Match</div>
            <div style={{ color: '#94A3B8', fontSize: 12 }}>Track live scores for any sport</div>
          </div>
        </div>
        <button
          type="button"
