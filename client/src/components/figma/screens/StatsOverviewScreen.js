...
import { Trophy, Clock, Target, Zap, ArrowRight, TrendingUp, BarChart3 } from 'lucide-react';

const SPORT_OPTIONS = ['All Sports', 'Badminton', 'Tennis', 'Football', 'Cricket'];

const stats = { totalGames: 42, hoursPlayed: 87, winRate: 68, streak: 5 };
const recentActivity = [
  { sport: 'Badminton', venue: 'Elite Sports Arena', date: 'Feb 20', result: 'Win', score: '21-18, 21-19' },
  { sport: 'Tennis', venue: 'Phoenix Tennis Club', date: 'Feb 18', result: 'Loss', score: '4-6, 6-7' },
  { sport: 'Football', venue: 'Champions Football Arena', date: 'Feb 15', result: 'Win', score: '3-2' },
];
...
const achievements = [
  ...
  { id: 2, title: 'Streak Master', description: '5 games in a row', icon: '🔥', unlocked: true },
  { id: 3, title: 'Social Player', description: 'Join 20 open play sessions', icon: '🤝', unlocked: false },
  { id: 4, title: 'Century', description: '100 hours played', icon: '💯', unlocked: false },
];

export function StatsOverviewScreen({ onViewLeaderboard, onViewMatchList, onViewSportAnalytics }) {
    ...
    <div className="figma-page">
      ...
      <div className="figma-grid2 figma-gap4" style={{ marginBottom: '2rem' }}>
        ...
        <div className="figma-card" style={{ padding: 16 }}>
          ...
          <div className="figma-body">Win Streak</div>
        </div>
      </div>
      ...
      <div className="figma-card" style={{ padding: 16, marginBottom: '2rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <span style={{ color: '#fff', fontSize: 16, fontWeight: 600 }}>Performance</span>
          <span style={{ color: '#94A3B8', fontSize: 12 }}>Last 30 days</span>
        </div>
        <div style={{ height: 120, background: 'var(--figma-bg)', borderRadius: 12, display: 'flex', alignItems: 'flex-end', justifyContent: 'space-around', padding: 12, gap: 8 }}>
          {[65, 45, 80, 55, 70, 60, 75].map((h, i) => (
            <div key={i} style={{ flex: 1, maxWidth: 24, height: `${h}%`, background: 'var(--figma-primary)', borderRadius: 6 }} title={`${h}%`} />
          ))}
        </div>
      </div>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <div className="figma-flex-between" style={{ marginBottom: 16 }}>
          <h2 className="figma-heading2">Leaderboard</h2>
          <button type="button" className="figma-link" onClick={() => onViewLeaderboard && onViewLeaderboard()}>
            View all <ArrowRight size={16} />
          </button>
        </div>
        <div className="figma-card" style={{ padding: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: 'linear-gradient(135deg, #F59E0B, #D97706)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>1</div>
            <div style={{ flex: 1 }}>
              <div style={{ color: '#fff', fontSize: 15, fontWeight: 600 }}>Rahul K.</div>
              <div style={{ color: '#94A3B8', fontSize: 12 }}>2,840 pts</div>
            </div>
            <TrendingUp size={16} color="#22C55E" />
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: 'var(--figma-card)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94A3B8', fontSize: 14, fontWeight: 700 }}>8</div>
            <div style={{ flex: 1 }}>
              <div style={{ color: '#fff', fontSize: 15, fontWeight: 600 }}>You</div>
              <div style={{ color: '#94A3B8', fontSize: 12 }}>1,720 pts</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <div className="figma-flex-between" style={{ marginBottom: 16 }}>
          <h2 className="figma-heading2" style={{ margin: 0 }}>Match History</h2>
          {onViewMatchList && (
            <button type="button" className="figma-link" onClick={() => onViewMatchList()}>Score match <ArrowRight size={16} /></button>
          )}
        </div>
        <div className="figma-space-y-4">
          {recentActivity.map((a, i) => (
            <div key={i} className="figma-card figma-flex-between" style={{ padding: 16 }}>
              <div>
                <div style={{ color: '#fff', fontSize: 16, fontWeight: 600 }}>{a.sport} · {a.venue}</div>
                <div style={{ color: '#94A3B8', fontSize: 14, marginTop: 4 }}>{a.date} — {a.score}</div>
              </div>
      ...
      <div>
        ...
        <div className="figma-grid2 figma-gap4">
          {achievements.map((ach) => (
            <div key={ach.id} className="figma-card figma-flex-col figma-gap2" style={{ padding: 16, opacity: ach.unlocked ? 1 : 0.5 }}>
