...
import type { TennisConfig, TennisState } from "./tennis";
import { inferTennisServe } from "./tennis";

// ─── Engine logic (re-used from tennis) ───────────────────────────────────────
...
function applyPoint(state: TennisState, team: "A" | "B"): TennisState {
  ...
  if (gameWinner) s = winGame(s, gameWinner);
  return s;
}
...
export const padelEngine: ScoringEngine<TennisState> = {
